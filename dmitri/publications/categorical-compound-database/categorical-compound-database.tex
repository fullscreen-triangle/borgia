...
\begin{equation}
\St = \frac{\log(\omega_{\mathrm{vib}}/\Brot)}{\log(\omega_{\mathrm{ref,max}}/\Brot^{\mathrm{ref}}_{\mathrm{min}})}
\label{eq:s_t_diatomic}
\end{equation}
where $\Brot^{\mathrm{ref}}_{\mathrm{min}} = 0.39$ cm$^{-1}$ is a large-moment-of-inertia reference value.
\end{definition}
...
Given coordinates $(\Sk, \St, \Se) \in [0,1]^3$, the $k$-trit interleaved ternary address $\mathbf{t} = (t_1, t_2, \ldots, t_k)$ is generated by Algorithm~\ref{alg:encoding}.
\end{definition}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{figures/panel_4_structure_prediction.png}
\caption{Structure prediction: property constraints define a region in S-entropy space, and the trie returns all compounds whose addresses fall within that region. This inverts the traditional cheminformatics workflow.}
...
\item Generate the interleaved ternary string by Algorithm~\ref{alg:encoding}.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{figures/panel_1_sentropy_space.png}
\caption{Distribution of 39 compounds in S-entropy coordinate space $(\Sk, \St, \Se)$. Diatomics (all with $\Se = 0$) lie in the $\Sk$-$\St$ plane at the base. Polyatomics occupy the interior of the cube with $\Se > 0$. Colour indicates molecular type.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{figures/panel_2_ternary_encoding.png}
\caption{Ternary encoding of the S-entropy space. Each trit subdivides one coordinate dimension into thirds. The interleaved sequence refines $\Sk$, $\St$, $\Se$ in rotation, progressively narrowing the addressed cell. Shown for a representative molecule.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{figures/panel_3_fuzzy_search.png}
\caption{Fuzzy search via prefix matching. Shorter prefixes address larger cells, returning more compounds. The resolution cascade from depth 3 to depth 12 progressively narrows the search, from broad chemical families to individual compounds.}
