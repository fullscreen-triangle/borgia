...
def main():
    ...
    plt.tight_layout()
    plt.savefig('gonfanolier/results/spectral_analysis.png', dpi=300)
    plt.show()
    ...
    summary = {
        ...
        'avg_peaks': total_peaks / len(results),
        'detection_rate': sum(1 for r in results if r['peaks'] > 0) / len(results)
    }
    
    with open('gonfanolier/results/spectral_analysis_results.json', 'w') as f:
