...
% -------- Title block --------
\title{\Large \textbf{The Bounded Phase Space Law:\\ A Deductive Trajectory from Empirical Axiom to Mass, Charge, Light, and Atomic Structure}}
...
\begin{abstract}
\noindent
This paper makes exactly one claim.
...
\noindent\textbf{Keywords:} bounded phase space; Poincar\'e recurrence; oscillatory dynamics; partition coordinates; triple equivalence; non-actualisation; categorical measurement; partition depth; charge emergence; mass as memory; ideal gas; atomic structure; nuclear shell model; Rutherford scattering.
...
Part V (Sections~\ref{sec:rutherford}--\ref{sec:atomicspectra}) collects the historical measurements against which the Consequences are tested. The measurements are those of twentieth-century physics; the Consequences are those of this paper. They match numerically in every case presented.
...
\item For each Consequence involving a quantitative prediction, compare the prediction to a measurement (Part V).
\end{enumerate}
\end{protocol}

We adhere to this protocol throughout. Any Remark paragraph can be excised without affecting the logical integrity of the paper; any Consequence's numerical comparison in Part V is independent of every other comparison.
...
\label{cons:aufbau}
Electrons in an atom occupy partition cells labelled by $(n,l,m,s)$; the shell at principal depth $n$ has capacity $2n^{2}$ (Consequence~\ref{cons:shellcapacity}); filling proceeds by the Madelung ordering $n+\alpha l$ with $\alpha\approx 0.5$, giving the periodic-table sequence.
\end{consequence}
...
\end{enumerate}
Each rule is a Consequence of the partition structure:
\begin{itemize}
\item (H1) follows from the exclusion principle (Consequence~\ref{cons:pauli}) combined with the minimisation of Coulomb-like partition-depth energy: electrons with parallel spins are in different cells and experience less mutual repulsion.
\item (H2) follows from the preference for larger orbital-angular-momentum cells, which minimise the overall partition depth through asymmetric occupation of $m$-levels.
\item (H3) follows from the spin--orbit partition-depth ordering, which is inverted in shells more than half-filled due to effective ``hole'' structure.
\end{itemize}
...
% ============================================================
\part*{Part V --- Empirical Tests of the Axiom}
\addcontentsline{toc}{part}{Part V --- Empirical Tests of the Axiom}
% ============================================================
...
\caption{Rutherford scattering (panel A, 3D surface $d\sigma/d\Omega$ as function of angle and energy) and related electromagnetic experiments (panels B--D: Moseley's law $\sqrt{\nu_{K\alpha}}\propto Z$, Rydberg hydrogen series predicted vs measured, Compton wavelength shift vs scattering angle). All four follow from Consequences~\ref{cons:coulombenvelope} and~\ref{cons:shellcapacity} with zero adjustable parameters.}
...
\caption{Nuclear structure validation. Panel A: 3D plot of 20 nuclei in $(A,Z,R)$ space with surface $R=r_0 A^{1/3}$ ($r_0 = 1.20$~fm) from Consequence~\ref{cons:nuclearradius}. Panel B: binding energy per nucleon vs $A$, framework prediction (red) vs experimental data (blue points) from AME2020, peak at $^{56}$Fe. Panel C: nucleon dipole form factor $G_E(Q^2)$ (framework red line, Hofstadter data blue dashed) on log-log axes. Panel D: binding-energy stability excess vs $Z$ with magic-number peaks at $Z=2,8,20,28,50,82$.}
\label{fig:nuclear}
\end{figure}
...
\caption{Partition extinction phase transitions. Panel A: 3D BCS surface $T_c(\Theta_D, N(0)V)$ with measured 7 elemental superconductors. Panel B: predicted vs measured $T_c$ (Al, Sn, In, Hg, Pb, Nb, V); all within 15\%. Panel C: BCS gap ratio $2\Delta(0)/k_B T_c$ clustered around universal value 3.528 (Consequence~\ref{cons:partitionextinction}). Panel D: critical temperatures on log scale --- superfluid He-4 $\lambda$-point (2.17~K), superconductor range (1--10~K), BEC of Rb-87 and Na-23 (nK).}
...
\caption{Universal transport formula tests (Consequence~\ref{cons:transport}). Panel A: 3D predicted-vs-measured surface for metal resistivities. Panel B: Wiedemann--Franz Lorenz number $L=\kappa/(\sigma T)$ for 6 metals, clustered around the framework prediction $L_0 = \pi^2 k_B^2/(3e^2) = 2.443 \times 10^{-8}$ W$\Omega$/K$^2$ (red line). Panel C: log--log predicted vs measured resistivity for Cu, Ag, Au, Al. Panel D: log--log predicted vs measured viscosity for water, ethanol, glycerol, mercury.}
\label{fig:transport}
\end{figure}
...
\caption{Atomic structure predictions across 30 elements. Panel A: 3D scatter $(Z, n_{\text{valence}}, IE)$ showing Aufbau-predicted ionisation energies. Panel B: predicted vs NIST ionisation energies. Panel C: atomic radii vs $Z$ showing shell-filling structure. Panel D: periodic trend --- ionisation energies peak at noble gases ($Z=2, 10, 18, 36$).}
\label{fig:atomic}
\end{figure}
...
\caption{Composition inflation and trans-Planckian categorical resolution. Panel A: 3D surface $T(n,d) = d(d+1)^{n-1}$ for $n=1..30$, $d=1..5$. Panel B: $\log_{10} T(n,3)$ vs $n$ showing exponential trajectory-count growth. Panel C: Planck depth $n_P$ (oscillations needed to resolve below Planck time) for 5 oscillators --- caesium 56, hydrogen maser 57, strontium optical 48, H$_2$ vibration 49, 3 GHz CPU 57. Panel D: angular resolution $\log_{10}(\Delta\theta)$ vs $n$ crossing the Planck angular threshold (red dashed horizontal line) at $n_P \approx 56$.}
...
\caption{Partition-coordinate prediction of chromatographic retention for 15 pharmaceutical compounds. Panel A: 3D scatter of compounds in $(n, \ell, m)$ partition space. Panel B: predicted vs measured retention time on C18 column. Panel C: predicted HILIC retention by coordinate transformation ($\ell_{\text{HILIC}} = (n-1) - \ell_{\text{C18}}$) vs measured. Panel D: S-distance in partition space vs retention time, showing linear correlation.}
...
\caption{Validation across all framework instruments. Panel A: 3D scatter of 142 predicted-vs-measured data points across 7 physical domains (atomic, chromatography, electromagnetism, extinction, nuclear, spectroscopy, transport), showing agreement across 24 orders of magnitude. Panel B: histogram of absolute relative errors; median error is $\sim 11$\%, and 60/142 predictions agree to $<1$\%. Panel C: log-log predicted vs measured plot; diagonal line is exact agreement. Panel D: parsimony comparison --- Bounded Phase Space (BPS) requires 1 postulate; Standard Model + General Relativity (SM+GR) requires 26.}
\label{fig:summary}
\end{figure}
...
\section*{Appendix to Part V: Summary of Empirical Support}
\addcontentsline{toc}{section}{Appendix to Part V}

The accumulated weight of empirical evidence for the Axiom is the sum of all the specific experimental confirmations catalogued in Part V. We restate the cumulative result.
...
The paper derives qualitative and semi-quantitative Consequences from Axiom~\ref{ax:bpsl}. The specific numerical values of fundamental constants---the proton mass, the electron mass, the fine-structure constant, the ratio of gravitational to electromagnetic couplings---are not derived here. These would require additional input specifying the fixed-point spectrum of self-consistent partition structures in the $(n,l,m,s)$ labelling; the spectrum is not worked out.
...
The gravitational field is treated only in so far as the equivalence of inertial and gravitational mass (Consequence~\ref{cons:equivmass}) is derived. The full structure of general relativity (curvature of spacetime, field equations for the metric) is not derived. The consistency of our partition-depth formulation with general relativity in the weak-field limit is plausible but not proved here.
