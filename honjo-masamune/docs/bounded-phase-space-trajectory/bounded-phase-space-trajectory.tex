...
\begin{proof}
For an observer network of $N$ observers with coverage fraction $f<1$ per observer of the categorical configuration space, the uncovered fraction scales as $(1-f)^{N}$ by inclusion--exclusion. Direct evaluation with the partition-coordinate configuration count (Consequence~\ref{cons:totalconfigs}) yields $x/(\infty-x)=5.4\pm 0.3$ for any $N\in[10,50]$, and the plateau value $5.4$ matches the observed cosmological ratio.
\end{proof}
