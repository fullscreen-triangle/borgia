...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{ion_journey.png}
...
\begin{figure*}[!htbp]
\centering
\includegraphics[width=0.48\textwidth]{ion_journey_H11_BD_A_neg_hilic.png}
\hfill
\includegraphics[width=0.48\textwidth]{ion_journey_PL_Neg_Waters_qTOF.png}
\vspace{0.5em}
\includegraphics[width=0.48\textwidth]{ion_journey_TG_Pos_Thermo_Orbi.png}
\hfill
\includegraphics[width=0.48\textwidth]{ion_journey_BSA1.png}
\caption{\textbf{Cross-Platform Ion Journeys.} Mass computing demonstrated on four datasets spanning different instrument platforms, ionization modes, and compound classes. Despite hardware differences, all ions follow the same eight-stage journey through S-entropy space. The ternary address framework provides platform-independent molecular state description.}
\label{fig:ion_journeys_platforms}
\end{figure*}
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{template_flow.png}
\caption{\textbf{Template-Based Analysis Flow.} At each pipeline stage, the ion (red) passes through a template mold (gray ring). Checkmarks indicate successful matching. The sequence of matches determines the complete ternary address. Non-matching ions fail validation and are excluded from downstream analysis.}
\label{fig:template_flow_main}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{panel_summary.png}
\caption{\textbf{Pipeline Validation Summary.} Left: Droplet wave encoding showing bijective transformation. Center: S-entropy projection $(S_k, S_e)$ colored by phase coherence. Right: Validation metrics confirming 100\% information preservation and physics quality.}
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{panel_1_molecular_flow.png}
\caption{\textbf{Molecular Flow Cascade.} (a) 3D S-entropy trajectory showing ion population. (b) Droplet cascade with velocity and radius encoding. (c) Wave pattern from droplet impact. (d) Bijective transformation flow: Ion $\to$ S-Entropy $\to$ Droplet $\to$ Wave Image.}
\label{fig:molecular_flow_impl}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{panel_3_thermodynamic.png}
\caption{\textbf{Thermodynamic Validation.} (a) Weber-Reynolds-Ohnesorge space showing droplet regime classification. (b) Information preservation at 100\% across all spectra. (c) Categorical state distribution. (d) Physics quality scores confirming valid droplet parameters.}
\label{fig:thermo_validation_impl}
\end{figure}
...
The intermediate dynamics are bypassed. We do not compute what happens; we read what the partition structure necessitates.
